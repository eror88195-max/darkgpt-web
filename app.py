...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', system-ui, sans-serif; }
        ...
        .main { flex: 1; display: flex; flex-direction: column; height: 100vh; background: #0e0e16; }
        .header { padding: 16px 28px; border-bottom: 1px solid #1a1728; display: flex; justify-content: space-between; align-items: center; background: #0b0b12; }
        .header .title { font-size: 20px; font-weight: 700; color: #d4c8f0; }
        .header .title span { color: #b388ff; }
        .header .badge { background: #1e1a2e; padding: 6px 18px; border-radius: 40px; font-size: 12px; color: #9c8bb5; border: 1px solid #2a1f3a; }
        .chat-area { flex: 1; overflow-y: auto; padding: 20px 28px 12px 28px; display: flex; flex-direction: column; gap: 16px; }
        ...
        .input-area { padding: 14px 28px 22px 28px; background: #0b0b12; border-top: 1px solid #1a1728; display: flex; align-items: center; gap: 14px; }
        .input-area .input-wrapper { flex: 1; position: relative; display: flex; align-items: center; background: #14111f; border-radius: 60px; border: 1px solid #2a1f3a; transition: 0.25s; padding: 4px 6px 4px 18px; }
        ...
        .input-area .input-wrapper .robot-icon:active { transform: scale(0.95); }
        .features-bar { display: flex; justify-content: center; gap: 28px; padding: 14px 20px 20px 20px; background: #0b0b12; border-top: 1px solid #12121c; flex-wrap: wrap; }
        ...
        @media (max-width: 760px) { .sidebar { display: none; } .main { width: 100%; } .header .badge { display: none; } .message { max-width: 92%; } .features-bar { gap: 12px; } .features-bar .feature-item { font-size: 11px; } .input-area { padding: 12px 16px 16px 16px; } .input-area .input-wrapper input { font-size: 14px; padding: 10px 8px; } .input-area .input-wrapper .robot-icon { width: 42px; height: 42px; font-size: 18px; } }
...
<div class="main">
    ...
    <div class="chat-area" id="chatArea">
        <div class="message bot"><span class="icon-bot"><i class="fas fa-robot"></i></span> مرحباً! أنا DarkGPT، ذكاء اصطناعي بلا حدود. طورني فريق ليوز. اسألني ما تشاء.</div>
        <div class="message user">مرحباً، كيف حالك؟</div>
        <div class="message bot"><span class="icon-bot"><i class="fas fa-robot"></i></span> أنا بخير، شكراً لسؤالك! أنا هنا لمساعدتك بكل ما تحتاجه، بدون أي قيود.</div>
    </div>
...
function resetChat() {
    const chatArea = document.getElementById('chatArea');
    chatArea.innerHTML = `<div class="message bot"><span class="icon-bot"><i class="fas fa-robot"></i></span> تم مسح المحادثة. كيف يمكنني مساعدتك اليوم؟</div>`;
